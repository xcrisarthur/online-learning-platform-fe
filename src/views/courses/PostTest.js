...
const PostTest = () => {
    ...
    if (userRole === 'student') {
      const handleVisibilityChange = () => {
        if (document.visibilityState === 'hidden' && isVisible) {
          var timeCountSuspend = 60
          setShowTabWarningModal(true)
          setIsVisible(false)
          setIsTestEnded(true)
          setIsButtonDisabled(true)
          setCountdown(timeCountSuspend)

          // Set countdown and start time in localStorage
          localStorage.setItem('countdown', timeCountSuspend)
          localStorage.setItem('startTime', Date.now().toString())
  ...
  return (
    <>
      ...
      <CModal
        visible={showTabWarningModal}
        onClose={() => setShowTabWarningModal(false)}
        alignment="center"
        aria-modal="true" // Tambahkan ini
        role="alertdialog" // Tambahkan ini
        backdrop="static" // Tambahkan ini
        keyboard={false} // Tambahkan ini
      >
        <CModalHeader className="bg-danger text-white">
          <CModalTitle>
            <div className="d-flex align-items-center">
        ...
        <CModalBody className="text-center py-4">
          ...
          <h5 className="mb-3">Tes Diberhentikan!</h5>
          <p>
            Anda tidak diperbolehkan berpindah tab selama tes berlangsung. Tes telah dihentikan
            secara otomatis untuk menjaga integritas penilaian.
          </p>
          <p className="text-muted mt-2">Silakan tunggu {countdown} detik untuk memulai kembali.</p>
        </CModalBody>
        <CModalFooter className="justify-content-center border-top-0">
          <CButton
            color="danger"
            onClick={() => setShowTabWarningModal(false)}
            tabIndex={0} // Tambahkan ini
            autoFocus // Tambahkan ini
          >
            Saya Mengerti
          </CButton>
        </CModalFooter>
      ...
      <div>
                ...
                <div className="mt-3">
                  <CAccordion flush>
                    <CAccordionItem itemKey={1}>
                      <CAccordionHeader>Tambah</CAccordionHeader>
                      <CAccordionBody>
                        <CContainer>
        ...
        {userRole === 'student' && (
          <div className="mb-3">
            <h2>Hi, {username}</h2>
            <p>Selamat datang di bagian Tes Akhir!</p>
            <p>
              Tes ini merupakan tes terakhir dari modul yang telah Anda pelajari. Tujuan dari tes
              ini adalah untuk mengevaluasi sejauh mana pemahaman Anda terhadap materi yang telah
              diberikan.
            </p>
            <p>
              Hasil tes ini akan menjadi penentu apakah Anda dapat melanjutkan ke materi berikutnya.
              Oleh karena itu, kerjakan tes ini dengan jujur dan sebaik-baiknya. Jangan khawatir
              dengan hasil tes, yang penting adalah Anda berusaha semaksimal mungkin.
            </p>
            <p>
              Perlu diperhatikan bahwa tes ini hanya dapat dilakukan SEKALI saja. Tidak ada
              kesempatan untuk mencoba ulang. Jadi, pastikan Anda mengerjakan dengan
              sungguh-sungguh.
            </p>
            ...
            <p>Selamat mengerjakan! Semoga Anda berhasil.</p>
            ...
            {userRole === 'student' && !isVisible ? (
              <div>
                <CLoadingButton
                  ...
                  timeout={1000}
                  disabled={loading || isTestEnded || isButtonDisabled || testResults.length > 0}
                >
                  {loading
                    ? 'Memuat...'
                    : testResults.length > 0
                ...
                {testResults.length > 0 && (
                  <div className="text-muted mt-2">
                    <small>
                      Anda telah menyelesaikan tes ini. Percobaan lebih dari satu kali tidak
                      diperbolehkan.
                    </small>
                  </div>
                )}
